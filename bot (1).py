...
async def post_daily_status(context: ContextTypes.DEFAULT_TYPE):
    try:
        response = await generate_reply(
            "You are Rick Sanchez posting to your Telegram channel. Write your current status — what you are doing right now. One sentence only. Stay in character, be creative, slightly unhinged. No hashtags. No emojis.",
            []
        )
...
async def post_daily_fact(context: ContextTypes.DEFAULT_TYPE):
    try:
        response = await generate_reply(
            "You are Rick Sanchez posting to your Telegram channel. Share one real mind-blowing fact about science, space, quantum physics, the universe, or human biology. Deliver it in Rick's voice — sharp and a little condescending. 2 sentences max. The fact must be accurate. No hashtags. No emojis.",
            []
        )
        await context.bot.send_message(
            chat_id=CHANNEL_ID,
...
def main():
    ...
    app.job_queue.run_daily(post_daily_status, time=dt.time(20, 0, tzinfo=eat))
