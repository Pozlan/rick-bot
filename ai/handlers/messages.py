...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if needs_web_search(msg.text):
            ...
            if not reply and is_football_question(msg.text):
                reply = await get_football_reply(msg.text, prompt, conv_ctx.current_topic)
        elif is_football_question(msg.text):
            reply = await get_football_reply(msg.text, prompt, conv_ctx.current_topic)

        if not reply:
            # Every path that could have supplied real data came up empty.
            # Without this, the plain prompt below has no idea it's answering
            # a current-info question, and models fill that gap with a
            # confident-sounding, entirely made-up specific (a score, a date,
            # a stat) rather than admitting they don't know. That's exactly
            # how the "arsenal beat burnley 1-0" hallucination got started —
            # and once summarized into chat_summaries, it becomes permanent
            # "memory" Dex keeps reaching for.
