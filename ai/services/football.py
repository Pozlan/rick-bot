"""
Football data via football-data.org (free tier, forever-free per the docs):
top 5 leagues + Champions League + World Cup + European Championship, plus
a real top-scorers endpoint for player stats.

This replaces the old TheSportsDB integration, which only ever covered
Real Madrid's own matches — it had no path to answer league-table, World
Cup, or player-stat questions at all. football-data.org actually covers
that ground.

Requires FOOTBALL_API_KEY (already in config.py, previously unused) —
get a free key at https://www.football-data.org/client/register (no card).

Free tier is rate-limited to 10 requests/minute, so every fetch here is
cached for CACHE_TTL_SECONDS — standings/scorers don't change fast enough
to need a fresh hit on every message anyway.
"""
import logging
import time
...
# Codes confirmed from their own docs — these are the only ones the free
# tier actually returns data for for. Anything else (Copa America, AFCON,
# domestic cups) needs a paid add-on and isn't wired up.
COMPETITION_CODES = {
    "PL":  "Premier League",
    "PD":  "La Liga",
    "SA":  "Serie A",
...
# Keywords/team names that resolve a message to a specific competition code.
# Order matters slightly (checked in dict order) but overlap is rare enough
...
def resolve_competition(text: str, topic: str = "") -> str:
    """
    Figure out which competition a question is about. Checks the message
    first, then falls back to the conversation topic (same pattern as
    services/web_search.py's topic-aware queries) for ambiguous follow-ups
    like "did they win" with no league named in the message itself.
    Returns a competition code, or "" if nothing matches.
    """
...
async def fetch_standings(code: str, top_n: int = 6) -> str:
    """League table for a competition. Not meaningful for cup/knockout
    competitions like World Cup/Champions League past the group stage —
    those still return group standings where applicable."""
...
async def fetch_recent_matches(code: str, limit: int = 5) -> str:
    """Most recent finished matches in a competition."""
...
async def fetch_scorers(code: str, limit: int = 5) -> str:
    """Top scorers — this is the player-stats source. Free tier gives goals/
    assists/appearances only; no minutes, cards, or lineup-level stats."""
...
async def fetch_todays_matches() -> str:
    """Generic fallback: matches today across whatever competitions the free
    tier covers. Used when a football question doesn't name a specific league."""
...
async def get_football_reply(message_text: str, prompt: str, topic: str = "") -> str:
    """
    Fetch relevant football data and have Dex comment on it naturally.
    Routes to the right data based on what's actually being asked:
    a named league/competition, player stats, or a generic "what happened" catch-all.
    """
