...
async def get_search_reply(message_text: str, prompt: str, topic: str = "") -> str:
    ...
    `topic` comes from ConversationContext.current_topic. A lot of real messages
    are ambiguous on their own — "what new album", "did they win", "is it out yet" —
    and only make sense with the subject the conversation was already about.
    Without this, DuckDuckGo gets a query with no subject and returns garbage,
    which silently falls through to the LLM guessing (and hallucinating).
    """
    try:
        query = message_text
