...
from config import USER_DATA_FILE, SUMMARIES_FILE, DATA_DIR, BASE_DIR, POSTED_FACTS_FILE
...
def load_posted_facts() -> list:
    try:
        with open(POSTED_FACTS_FILE, "r") as f:
            return json.load(f)
    except Exception:
        return []


def save_posted_facts(facts: list) -> None:
    try:
        with open(POSTED_FACTS_FILE, "w") as f:
            json.dump(facts, f)
    except Exception as e:
        logger.error(f"Failed to save posted facts: {e}")
